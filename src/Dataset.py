...
class MultiplicationDataset(Dataset):
    ...
    # Fixed seeds to ensure consistent datasets across runs
    _fixed_seed = 42
    
    @classmethod
    def set_fixed_seed(cls, seed):
        """Set a fixed seed for all dataset instances to ensure consistency"""
        cls._fixed_seed = seed
    
    @classmethod
    def _generate_systematic_problems(cls, min_value, max_value, split_ratio=(0.8, 0.1, 0.1), seed=None):
        """Generate all possible multiplication problems and split them systematically"""
        # Use the fixed seed if none provided
        if seed is None:
            seed = cls._fixed_seed
        ...
        if config_key in cls._all_problems:
            return (cls._train_problems[config_key], 
        ...
        num_buckets = min(10, len(all_problems))  # Divide into 10 difficulty levels, but not more than problems
        bucket_size = max(1, len(all_problems) // num_buckets)  # Ensure bucket_size is at least 1
        buckets = [all_problems[i:i+bucket_size] for i in range(0, len(all_problems), bucket_size)]
        ...
        for bucket in buckets:
            ...
            val_problems.extend(bucket[bucket_train_size:bucket_train_size+bucket_val_size])
    ...
    def __init__(self, num_samples: int, split: str = 'train', split_ratio: Tuple[float, float, float] = (0.8, 0.1, 0.1), seed: int = None, min_value: int = None, max_value: int = None):
        """
        ...
        Args:
            ...
            seed: Random seed for reproducibility (if None, uses class fixed_seed)
            min_value: Minimum value for multiplication operands
            max_value: Maximum value for multiplication operands
        """
        # Use the class fixed seed if none provided to ensure consistency
        if seed is None:
        ...
        print(f"Using {split} dataset with range {self.min_value}-{self.max_value}: {self.num_samples} problems")
