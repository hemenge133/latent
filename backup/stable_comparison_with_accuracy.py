...
class StableSimpleTransformer(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        nn.init.normal_(
            self.pos_encoder, mean=0, std=0.01
        )  # Smaller std for better stability
    ...
    def forward(self, src, tgt):
        ...
        # Apply decoder
        output = self.decoder(
            tgt_embed, memory, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_padding_mask
        )
...
class StableLatentTransformer(nn.Module):
    def __init__(
        ...
        self.bottleneck_factor = (
            bottleneck_factor  # Add this parameter back from original
        )
    ...
    def _reset_parameters(self):
        ...
        nn.init.normal_(
            self.latent_tokens, mean=0.0, std=0.01
        )  # Smaller std for better stability

        # Initialize positional encodings
        nn.init.normal_(
            self.pos_encoder, mean=0.0, std=0.01
        )  # Smaller std for better stability
    ...
    def _cross_attention(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            scores = scores.masked_fill(
                attn_mask == 0, -1e4
            )  # Using -1e4 instead of -1e9 to avoid FP16 overflow
    ...
    def forward(self, src, tgt):
        ...
        latent_attended = self._cross_attention(
            ...
            else ~src_padding_mask.unsqueeze(1),
        )
        ...
        if self.bottleneck_factor < 1.0:
            ...
            mixed_memory = torch.cat(
                [
                    ...
                    memory
                    * (1.0 - self.bottleneck_factor),  # [batch, seq_len, d_model]
                ],
                dim=1,
            )
        else:
...
def inference(model, input_tensor, dataset, max_length=10, device="cpu"):
    ...
    result_tokens = [
        t for t in output_sequence[1:] if t != 0
    ]  # Remove start token and padding
...
def train_model(
    ...
    def warmup_cosine_schedule(step):
        ...
        progress = float(step - config.warmup_steps) / float(
            max(1, max_steps - config.warmup_steps)
        )
    ...
    eval_examples = generate_multiplication_examples(
        dataset, num_examples=10, device=device
    )
            ...
            for batch_idx, (inp, tgt, inp_lens, tgt_lens) in enumerate(train_loader):
                ...
                if device.type == "cuda":
                    ...
                    with torch.cuda.amp.autocast():
                        output = model(inp, decoder_input)
                        loss = criterion(
                            output.reshape(-1, vocab_size), decoder_target.reshape(-1)
                        )
                ...
                if step > 0 and step % val_freq == 0:
                    ...
                    if val_loss < 0.01 and val_accuracy > 0.95:
                        print(
                            f"Reached excellent validation performance, stopping early"
                        )
                        break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stable comparison between transformer architectures with accuracy metrics"
    )
    parser.add_argument(
        "--d-model", type=int, default=64, help="Model dimension (default: 64)"
    )
    parser.add_argument(
        "--num-layers", type=int, default=2, help="Number of layers (default: 2)"
    )
    parser.add_argument(
        "--num-latent", type=int, default=4, help="Number of latent tokens (default: 4)"
    )
    parser.add_argument(
        "--bottleneck-factor",
        type=float,
        default=1.0,
        help="Bottleneck factor (default: 1.0)",
    )
    parser.add_argument(
        "--max-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum number of digits (default: 1)",
    )
    ...
    latent_log_dir = (
        f"runs/stable_comparison_acc/latent_d{d_model}_l{num_layers}_n{num_latent}"
    )
    ...
    print(
        f"SimpleTransformer: {simple_loss:.6f} loss, {simple_accuracy:.2%} accuracy, {simple_params:,} parameters, {simple_time:.2f}s training time"
    )
    print(
        f"LatentTransformer: {latent_loss:.6f} loss, {latent_accuracy:.2%} accuracy, {latent_params:,} parameters, {latent_time:.2f}s training time"
    )

    # Calculate efficiency metrics if possible
    if (
        simple_loss != float("inf")
        and latent_loss != float("inf")
        and not np.isnan(simple_loss)
        and not np.isnan(latent_loss)
    ):
        ...
        acc_param_efficiency_simple = (
            simple_accuracy / simple_params if simple_params > 0 else 0
        )
        acc_param_efficiency_latent = (
            latent_accuracy / latent_params if latent_params > 0 else 0
        )
