"""
Training script for LatentTransformer model
"""
import torch
...
def main():
    ...
    writer = SummaryWriter(log_dir=config.latent_transformer_logdir)
    ...
    save_dir = setup_training_dir(config.latent_transformer_name)
    ...
    for epoch in range(config.max_epochs):
            ...
            if hasattr(config, 'validate_every_n_steps') and global_step % config.validate_every_n_steps == 0:
                ...
                if val_loss_step < best_val_loss:
                    ...
                    save_checkpoint(
                        ...
                        save_path=f"{save_dir}/{config.latent_transformer_name}_best.pt",
                        is_best=True
                    )
        ...
        if (epoch + 1) % config.save_every == 0:
            save_checkpoint(
                ...
                train_loss=train_loss,
                val_loss=val_loss,
                save_path=f"{save_dir}/{config.latent_transformer_name}_epoch_{epoch+1}.pt"
            )
    ...
    checkpoint = load_checkpoint(
        model=model,
        optimizer=None,
        checkpoint_path=f"{save_dir}/{config.latent_transformer_name}_best.pt",
        device=device
    )
    test_loss = evaluate(
        model=model,
